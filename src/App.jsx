...
const App = (call) => {
    ...
    const fetchData = async () => {
        ...
        // let group = _.filter(json, {'is_archived': false});
        // setCalls(json);
        return json;
    }
    ...
    useEffect( () =>
        fetchData()
            .catch(console.error)
            .then(json => setCalls(_.filter(json, {'is_archived': false})))
    ,[]);



    function filterInbox() {
        // console.log(filterInbox());
        fetchData()
            .catch(console.error)
            .then(json => setCalls(_.filter(json, {'is_archived': false})));
        console.log(calls);
    ...
    function filterAll() {
        fetchData()
            .catch(console.error)
            .then(json => setCalls(json));
    }

    function filterArchived() {
        fetchData()
            .catch(console.error)
            .then(json => setCalls(_.filter(json, {'is_archived': true})));
    }
    ...
    <div>
        <Container>
            <div className="nav-container">
                <Row>
                    <Col xs={3}>
                        <svg className="logo" width='486px' height='168px' viewBox='0 0 486 168' version='1.1' xmlns='http://www.w3.org/2000/svg'>
                    ...
                    <Col xs={9}>
                        <ButtonGroup size="small" aria-label="medium secondary button group">
                            <Button onClick={filterInbox}>Inbox</Button>
                            <Button onClick={filterAll}>All calls</Button>
                            <Button onClick={filterArchived}>Archived</Button>
                        </ButtonGroup>
                    </Col>
                </Row>
            </div>
            {calls.map(item => (
            <CallRecord call={item} key={item.id}/>))}
        </Container>
